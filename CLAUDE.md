...
**Automazione** (`.github/workflows/fofi-update.yml`, vedi `AUTOMAZIONE-FOFI.md`, presente sia su `dev` che su `main` per far scattare il cron): workflow schedulato (lun-sab) che scarica la mail (`tools/fetch-fofi-email.mjs`, Gmail API read-only via secret `GMAIL_*`; da ~giugno 2026 FOFI non allega più l'Excel ma mette un **link Azure blob `rufblob…xlsx`** nel corpo — lo script prova l'allegato MIME e in fallback estrae/scarica quel link, vedi `extractXlsxUrl`/`collectBody`), aggiorna la banca dati, bumpa versione+cache (`tools/bump-build.mjs`), gira i test, apre una PR su `dev` e **la mergia automaticamente**, poi **propaga dev→main (produzione) senza conferma manuale** (deroga esplicita, richiesta dall'utente il 15/07/2026, alla regola generale "mai merge su main senza conferma esplicita" — vale solo per questa pipeline FOFI). `tools/bump-build.mjs` tiene allineati `APP_BUILD`/`APP_BUILD_DATE` (index.html) e `CACHE_NAME` (sw.js) — funzioni pure testate in `tests/bump-build.test.js`.
